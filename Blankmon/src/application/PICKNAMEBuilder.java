...
public class PICKNAMEBuilder
{
	public PICKNAMEmon createPICKNAMEmon(Species species, int level)
	{
		...
		try
		{
			Connection connect = DatabaseConnection.dbConnector(DatabaseType.PokemonDatabase);
		...
		return new PICKNAMEmon(species.toString(), "TODO PLAYER NAME HERE", level, 0, gender, moves, types, species, isShiny, ability,
	...
	private MoveSet generateMoveSet(Species species, int level)
	{
		...
		try
		{
			Connection connect = DatabaseConnection.dbConnector(DatabaseType.PokemonSpeciesDatabase);
